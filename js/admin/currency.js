    ...
    jQuery(function() { 
        ...
        jQuery( "#paccordion1" ).accordion({
            ...
            jQuery('#add_currency').live("click",function(){
               ...
               jQuery('#currency_table').append('<tr id="currency_row_'+tm+'"><td><span id="w8c_'+tm+'" style="position: absolute; text-decoration: blink; margin-left: 20px; display: none;">Saving...</span><input type="radio" name="currency_radio" class="currency_radio" id="'+tm+'"></td><td><input type="text" name="_odudes_settings[currency]['+tm+'][currency_name]" id="c_n_'+tm+'" value="'+jQuery('#currency_n').val()+'" class="currency_name"></td><td><input  class="currency_symbol" type="text" name="_odudes_settings[currency]['+tm+'][currency_symbol]" id="c_s_'+tm+'" value="'+jQuery('#currency_s').val()+'"></td><td><a href="#" class="del_currency" id="'+tm+'">Delete</a></td></tr>'); 
            ...
            jQuery('.del_currency').live("click",function(){
                ...
                jQuery.post(ajaxurl,{action:"odudes_default_currency_del",currency_name:jQuery('#c_n_'+jQuery(this).attr("id")).val(),currency_value:jQuery('#c_s_'+jQuery(this).attr("id")).val(),currency_key:jQuery(this).attr("id")},
                function(res){
            ...
            jQuery('.currency_radio').live("click",function(){
                ...
                jQuery(ccid).fadeIn();
                jQuery.post(ajaxurl,{action:"odudes_default_currency",currency_name:jQuery('#c_n_'+jQuery(this).attr("id")).val(),currency_value:jQuery('#c_s_'+jQuery(this).attr("id")).val(),currency_key:jQuery(this).attr("id")},function(res){jQuery(ccid).fadeOut();});
            });
