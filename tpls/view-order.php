...
<?php
    ...
    $oitems = $wpdb->get_results("select * from {$wpdb->prefix}os_order_items where oid='{$order->order_id}'");
    $user = new WP_User( $order->uid );
    $role = $user->roles[0];
    //$tax = $order1->odudes_calculate_tax($order->order_id);
...
<thead>
<tr><th align="left"><?php echo __("Item Name","odudeshop");?></th>
    <th align="left"><?php echo __("Unit Price","odudeshop");?></th>
    <th align="left"><?php echo __("Quantity","odudeshop");?></th>
   
    <th align="left"><?php echo __("Coupon Code","odudeshop");?></th>
    <th align="left"><?php echo __("Coupon Discount","odudeshop");?></th>
    <th align="left"><?php echo __("Total","odudeshop");?></th>
    <th align="left"><?php echo __("Subtotal","odudeshop");?></th>
</tr>
</thead>
<?php 
...
if(is_array($cart_data) && !empty($cart_data)):
    ...
    foreach ($cart_data as $pid => $item):
        if(isset($item['item'])):
            ...
            ?>
                <tr>
                    <td><?php echo "<a href='".$pro_link."' target='_blank'>".$item['post_title']." </a><br>" . $vari; ?></td>
		...
		?>
            <tr>
                ...
                <td><?php if(isset($item['coupon'])) echo $item['coupon']; ?></td>
...
<div class="wrap">
    ...
    <div id="msg" style="padding: 5px 10px;display: none;" class="message updated"><?php echo __("Message", "odudeshop"); ?></div>
    <div style="float: right;width:400px;">
        <table class="widefat fixed">
            <tr><th align="left" colspan="2"><?php echo __("Order Summary", "odudeshop"); ?></th></tr>
            <tr><td><?php echo __("Order ID:", "odudeshop"); ?></td><td><?php echo $order->order_id; ?></td></tr>
            <tr><td><?php echo __("Coupon Discount:", "odudeshop"); ?></td><td><?php echo $currency_sign . $coupon_discount; ?></td></tr>
           
        
            <tr><td><?php echo __("Shipping:", "odudeshop"); ?></td><td><?php echo $currency_sign . $order->shipping_cost; ?></td></tr>
            <?php $ret = '';
            $ret = apply_filters('odudes_admin_order_details',$ret,$order->order_id);
            if($ret != '') echo $ret;
            ?> 
            <tr><td><?php echo __("Order Total:", "odudeshop"); ?></td><td><?php echo $currency_sign . number_format($order->total, 2); ?></td></tr>
            <tr><td><?php echo __("Order Date:", "odudeshop"); ?></td><td><?php echo date("Y-m-d h:i a",$order->date); ?></td></tr>
<tr><td><?php echo __("Order Note:", "odudeshop"); ?></td><td><?php echo $order->order_notes; ?></td></tr>       
	   </table>
    </div>
    <div style="float: left;width: 500px;">
        <table class="widefat fixed">
            <tr><th align="left" colspan="2"><?php echo __("Customer Info", "odudeshop"); ?></th></tr>
            <tr><td><?php echo __("Customer ID:", "odudeshop"); ?></td><td><?php echo $user->ID; ?></td></tr>
            <tr><td><?php echo __("Customer Name:", "odudeshop"); ?></td><td><?php  echo '<a href="'. get_edit_user_link( $user->ID ) .'">'. esc_attr( $user->user_nicename ) .'</a>'; ?></td></tr>
            <tr><td><?php echo __("Customer Email:", "odudeshop"); ?></td><td><?php echo $user->user_email; ?></td></tr>
			<tr><td><?php echo __("Payment Status:", "odudeshop"); ?></td><td><?php echo $order->payment_status; ?></td></tr>
				<tr><td><?php echo __("Order Status:", "odudeshop"); ?></td><td><?php echo $order->order_status; ?></td></tr>
					<tr><td><?php echo __("Payment Method:", "odudeshop"); ?></td><td><?php echo $order->payment_method; ?></td></tr>
						<tr><td><?php echo __("Shipping Method:", "odudeshop"); ?></td><td><?php echo $order->shipping_method; ?></td></tr>
        </table>
    </div>
    <div style="clear: both;"></div>
    <h2 style="font-size: 12pt"><?php echo __("Order Items", "odudeshop"); ?></h2>

    <?php echo $content; ?>

    <br />
    <b><?php echo __("Order Status:", "odudeshop"); ?> 
    ...
    &nbsp;
    <b><?php echo __("Payment Status:", "odudeshop"); ?> 
...
<!--
  jQuery(function(){
      ...
      jQuery('#update_os').click(function(){
          jQuery('#lng').fadeIn();
          jQuery.post(ajaxurl,{action:'odudes_ajax_call',execute:'update_os',order_id:'<?php echo $_GET[id]; ?>',status:jQuery('#osv').val()},function(res){
      ...
      jQuery('#update_ps').click(function(){
          jQuery('#lng').fadeIn();
          jQuery.post(ajaxurl,{action:'odudes_ajax_call',execute:'update_ps',order_id:'<?php echo $_GET[id]; ?>',status:jQuery('#psv').val()},function(res){
              jQuery('#msg').html(res).fadeIn();
              jQuery('#lng').fadeOut();
          });
      });
      ...
      jQuery('#reduce_stock').click(function(){
          jQuery('#lng').fadeIn();
          jQuery.post(ajaxurl,{action:'odudes_ajax_call',execute:'odudes_reduce_stock',order_id:'<?php echo $_GET[id]; ?>'},function(res){
              jQuery('#msg').html(res).fadeIn();
              jQuery('#lng').fadeOut();
          });
      });
      ...
      jQuery('#restore_stock').click(function(){
          jQuery('#lng').fadeIn();
          jQuery.post(ajaxurl,{action:'odudes_ajax_call',execute:'odudes_restore_stock',order_id:'<?php echo $_GET[id]; ?>'},function(res){
              jQuery('#msg').html(res).fadeIn();
              jQuery('#lng').fadeOut();
          });
      });
