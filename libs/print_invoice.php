...
$pluginurl=plugins_url("odudeshop/images/");
$orderid_string=__("Order ID:","odudeshop");
$ordersummary_string=__("Order Summary","odudeshop");
$orderdiscount_string=__("Order Discount:","odudeshop");
$ordershipping_string=__("Shipping:","odudeshop");
$ordertotal_string=__("Order Total:","odudeshop");
$orderdate_string=__("Order Date:","odudeshop");
$customerid_string=__("Customer ID:","odudeshop");
$customerdetails_string=__("Customer Details","odudeshop");
$customername_string=__("Customer Name:","odudeshop");
$customeremail_string=__("Customer Email:","odudeshop");
$itemname_string=__("Item Name","odudeshop");
$unitprice_string=__("Unit Price","odudeshop");
$quantity_string=__("Quantity","odudeshop");
$discount_string=__("Discount","odudeshop");
$subtotal_string=__("Subtotal","odudeshop");
$download_string=__("Download","odudeshop");
$_ohtml = <<<OTH
<style type="text/css">
tr.odd {
...
if($count%2==1) $trclass="odd";else $trclass="even";
     ...
     $meta = get_post_meta($ditem->ID,'odudes_list_opts',true);
     ...
     $prices=0;
    $variations="";
    //print_r(get_post_meta($oitem->pid,"odudes_list_opts",true));
      
     @extract(get_post_meta($oitem->pid,"odudes_list_opts",true));
    ...
    $download_link = home_url("/?odudefile=$wpmpdl");  
...
if($order->payment_status!='Completed'){
    ...
    <tr class="items"><td colspan="4">Please complete your payment to view download links. <div id="proceed_{$order->order_id}" style="float:right">    
         ...
         function proceed2payment_{$order->order_id}(ob){
            ...
            jQuery.post('{$purl}',{task:'paymentfront',action:'odudes_ajax_call',execute:'PayNow',order_id:'{$order->order_id}',payment_method:jQuery('#pgdd_{$order->order_id}').val()},function(res){
...
window.print();
  function getkey(file, order_id){
      ...
      jQuery.post('{$homeurl}',{action:'odudes_ajax_call',execute:'getlicensekey',fileid:file,orderid:order_id},function(res){
