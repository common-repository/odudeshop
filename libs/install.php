...
if($whatsell=='d')
    ...
    register_post_type("odudeshop",array(
            
            'labels' => array(
                'name' => __('ODudeShop',"odudeshop"),
                'singular_name' => __('Product',"odudeshop"),
                'add_new' => __('Add '.$product.' Product',"odudeshop"),
                'add_new_item' => __('Add New '.$product.' Product',"odudeshop"),
                'edit_item' => __('Edit '.$product.' Product',"odudeshop"), 
                'new_item' => __('New '.$product.' Product',"odudeshop"),
                'view_item' => __('View Product',"odudeshop"),
                'search_items' => __('Search Product',"odudeshop"),
                'not_found' =>  __('No product found',"odudeshop"),
                'not_found_in_trash' => __('No product found in Trash',"odudeshop"), 
                'parent_item_colon' => ''
            ),
            'public' => true,
            ...
            'hierarchical' => false,
            'menu_icon' =>plugins_url().'/odudeshop/images/odude.png',
			//'supports' => array('title','editor','author','excerpt','thumbnail','ptype','comments'/*,'custom-fields'*/) ,            
            'supports' => array('title','editor','excerpt','ptype','comments'/*,'custom-fields'*/) ,
	...
	register_post_type( 'odudeshop_tabs',
		array(
			'labels' => array(
				'name' => __( 'ODudeShop Tabs' ),
				'singular_name' => __( 'Information Tab' ),
				'add_new_item' => __( 'Add New Tab' )
			),
			'public' => true,
           // 'show_in_menu' => 'edit.php?post_type=odudeshop',
			 'supports' => array('title','editor','tabcat') ,
			'has_archive' => true,
			 'menu_icon' =>plugins_url().'/odudeshop/images/odude.png',
			'taxonomies' => array('tabcat'),
			 'show_ui' => true, 
			 'exclude_from_search' => true,
			 'show_in_nav_menus'=>false,
			 'hierarchical' => true,
			 'capability_type' => 'post',
			'rewrite' => array('slug' => 'odudeshop_tabs','with_front'=>true),
			'show_in_menu'=>true
...
function register_ODudeShop_product_taxonomies()
{
  // Add new taxonomy, make it hierarchical (like categories)
  $labels = array(
    'name' => __( 'Product Categories',"odudeshop" ),
    'singular_name' => __( 'Product Category',"odudeshop"),
    'search_items' =>  __( 'Search Categories',"odudeshop" ),
    'all_items' => __( 'All Categories',"odudeshop" ),
    'parent_item' => __( 'Parent Category',"odudeshop" ),
    'parent_item_colon' => __( 'Parent Category:',"odudeshop" ),
    'edit_item' => __( 'Edit Category',"odudeshop" ), 
    'update_item' => __( 'Update Category',"odudeshop" ),
    'add_new_item' => __( 'Add New Category',"odudeshop" ),
    'new_item_name' => __( 'New Category Name',"odudeshop" ),
    'menu_name' => __( 'Product Categories',"odudeshop" ),
  );     

  register_taxonomy('ptype',array('odudeshop'), array(
    'hierarchical' => true,
    'labels' => $labels,
    'show_ui' => true,
    'query_var' => true,
	...
	register_taxonomy(
		'tabcat',
		'odudeshop_tabs',
		//'odudeshop', //this is wokring but will not display categories at information tab
		array(
			'label' => __( 'Tab Categories' ),
			 'show_ui' => true,
			'rewrite' => array( 'slug' => 'odudeshop-tabs' ),
			'query_var' => true,
			'hierarchical' => true,
		)
	);
...
function odudes_install()
{
	
	//add_role( 'odudeshop_customer', 'ODudeShop Customer', array( 'read' => true, 'level_0' => true ) );
	
    global $wpdb;
    require_once(ABSPATH . 'wp-admin/includes/upgrade.php');    
    $sql[] = "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}os_orders` (
  ...
  PRIMARY KEY (`order_id`)
    ...
    $sql[] = "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}os_order_items` (
  ...
  PRIMARY KEY (`id`)
    ...
    $sql[] = "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}os_payment_methods` (
          `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $sql[] = "INSERT INTO `{$wpdb->prefix}os_payment_methods` (`id`, `title`, `description`, `class_name`, `enabled`, `default`) VALUES(1, 'PayPal', 'PayPal', 'paypal', 1, 1)"; 

    $sql[] = "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}os_country` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`)
    ...
    $sql[] = "INSERT INTO `{$wpdb->prefix}os_country` (`id`, `country_code`, `country_name`, `status`) VALUES
        (1, 'AD', 'ANDORRA', NULL),
    ...
    $sql[] = "ALTER TABLE `{$wpdb->prefix}os_orders` ADD `order_notes` VARCHAR( 255 ) NULL ,
    ...
    $sql[] = "ALTER TABLE `{$wpdb->prefix}os_order_items` ADD `coupon` VARCHAR( 255 ) NULL ,
    ADD `coupon_amount` FLOAT NULL   ";

    $sql[] = "ALTER TABLE `{$wpdb->prefix}os_order_items` ADD `site_commission` float not NULL DEFAULT '0'";
    ...
    if(!$wpdb->get_var("select id from {$wpdb->prefix}posts where post_content like '%[odudes-cart]%'")){
        $cart_id = wp_insert_post(array('post_title'=>'Cart','post_content'=>'[odudes-cart]','post_type'=>'page','post_status'=>'publish'));
        $co_id = wp_insert_post(array('post_title'=>'Checkout','post_content'=>'[odudes-checkout]','post_type'=>'page','post_status'=>'publish'));
        wp_insert_post(array('post_title'=>'My Orders','post_content'=>'[my-orders]','post_type'=>'page','post_status'=>'publish'));
        $id = wp_insert_post(array('post_title'=>'Products','post_content'=>'[odudes-all-products perrow=2 perpage=10]','post_type'=>'page','post_status'=>'publish'));
        wp_insert_post(array('post_title'=>'section','post_content'=>'[odudes-all-section perrow=2 perpage=10 orderby="date"]','post_type'=>'page','post_status'=>'publish'));
    }
	$_odudes_settings['user-reg'] = 1;
    $_odudes_settings['whatsell'] = 'p';
	$_odudes_settings['showcart'] = 1;
	$_odudes_settings['disable_fron_end_css'] = 0;
    $_odudes_settings['generate_product_page_content'] = 1;
    $_odudes_settings['fstatus'] = 'publish';
    $_odudes_settings['page_id'] = $cart_id;
    $_odudes_settings['check_page_id'] = $co_id;
    $_odudes_settings['continue_shopping_url'] = site_url('product/');
    $_odudes_settings['odudes_after_addtocart_redirect'] = 0;
    $_odudes_settings['Paypal']['enabled'] = 1;
    $_odudes_settings['Paypal']['Paypal_mode'] = 'live';
    
    if(!get_option('_odudes_settings'))
        update_option('_odudes_settings',$_odudes_settings);
    
    update_option('odudes_access_level','level_2');      
    odudes_post_types();
    register_ODudeShop_product_taxonomies();
    flush_rewrite_rules(); 
}
?>