<?php
function odudes_show_cart()
{
    global $wpdb;
	$settings = get_option('_odudes_settings');
    odudes_calculate_discount();
    $cart_data = odudes_get_cart_data();
    //echo "<pre>"; print_r($cart_data); echo "</pre>";
	
		if(isset($settings['msg_cart_top']))
		  $msg_cart_top=$settings['msg_cart_top']."<br>" ;
	  else
		  $msg_cart_top="";
	
	
	 if($settings['showcart']==1)
    include(WP_PLUGIN_DIR."/odudeshop/tpls/cart.php");
	else
	include(WP_PLUGIN_DIR."/odudeshop/tpls/cart_enquiry.php");


    return $xcart . "<br>".$msg_cart_top;
}
...
function check_coupon($pid,$coupon){
    @extract(get_post_meta($pid,"odudes_list_opts",true));
...
function odudes_add_to_cart(){ 
    if(isset($_POST['add_to_cart']) && $_POST['add_to_cart']=="add"){        
        ...
        $pid = isset($_REQUEST['odudes_add_to_cart']) ? $_REQUEST['odudes_add_to_cart']:$_POST['pid'];
        $pid = apply_filters("odudes_add_to_cart", $pid);
        if($pid<=0) return;
        ...
        $base_price = odudes_product_price($pid);    
        //
        ...
        if(array_key_exists($pid, $cart_data)){
            ...
            if(isset($cart_data[$pid]['multi']) && $cart_data[$pid]['multi']==1){
                ...
                foreach ($product_data as $key => $item):
                    ...
                    if(odudes_array_diff($item['variation'], $_REQUEST['variation'])==true){
                ...
                if(odudes_array_diff($cart_data[$pid]['variation'] , $_REQUEST['variation'])==true){
			...
			$cart_data[$pid] = array('quantity'=>$q,'variation'=>'','price'=>$base_price);   	
        }
       ...
       // echo "<pre>";        print_r($cart_data); echo "</pre>";
        ...
        if($settings['odudes_after_addtocart_redirect']==1){
...
function odudes_remove_cart_item(){
    if(!isset($_REQUEST['odudes_remove_cart_item']) || $_REQUEST['odudes_remove_cart_item']<=0) return;    
    $cart_data = odudes_get_cart_data();
    if(isset($_REQUEST['item_id'])){
        unset($cart_data[$_REQUEST['odudes_remove_cart_item']]['item'][$_REQUEST['item_id']]);
        if(empty($cart_data[$_REQUEST['odudes_remove_cart_item']]['item'])) {
            unset($cart_data[$_REQUEST['odudes_remove_cart_item']]);
        }
    }
    else{   
        unset($cart_data[$_REQUEST['odudes_remove_cart_item']]);    
    ...
    odudes_update_cart_data($cart_data);
    $ret['cart_subtotal'] = odudes_get_cart_subtotal();
    $ret['cart_discount'] = odudes_get_cart_discount();
    $ret['cart_total'] = odudes_get_cart_total();
    die(json_encode($ret));
}

function odudes_update_cart(){
    if(!isset($_REQUEST['odudes_update_cart']) || (isset($_REQUEST['odudes_update_cart']) && $_REQUEST['odudes_update_cart']<=0)) return;
...
//    die();
    ...
    $cart_data = odudes_get_cart_data(); //get previous cart data
...
function odudes_update_cart_data($cart_data){
    //echo "<pre>";
    //print_r($cart_data);
    //die();
    global $current_user;
   ...
   if(isset($settings['stock']['enable']))
   if($settings['stock']['enable']==1)
   { 
        foreach($cart_data as $pid=>$cartitem){
            $post_meta=array();
            $post_meta=get_post_meta($pid,"odudes_list_opts",true);
...
function odudes_get_cart_items(){
    global $current_user, $wpdb;    
    $cart_data = odudes_get_cart_data();    
    return ($cart_data);
}

function odudes_calculate_discount(){
    //echo "I'm here ";
    global $current_user;
    get_currentuserinfo();
    ...
    foreach($cart_items as $pid=>$item)    {
        ...
        $opt = get_post_meta($pid,"odudes_list_opts",true);
        @extract($opt);
                ...
                foreach($value as $optionkey=>$optionvalue){
                  if($optionkey!="vname" && $optionkey != 'multiple'){
                      ...
                      else
					  {
                        ...
                        foreach((array)$item['variation'] as $var){                   
                            if($var==$optionkey){
    ...
    odudes_update_cart_data($cart_items);
                }
}
...
function wpmpz_get_cart_total(){
    ...
    $total = apply_filters('odudes_cart_subtotal',$total);
    return number_format($total,2,".","");
}


function odudes_get_cart_subtotal()
{
    $cart_items = odudes_get_cart_items();
...
function odudes_get_cart_discount(){
    global $current_user;
    get_currentuserinfo();
    $role = $current_user->roles[0];
    $role = $role?$role:'guest';
    //$subtotal = odudes_get_cart_subtotal();
    $cart_items = odudes_get_cart_items();
    $discount_r=0;
    ...
    foreach($cart_items as $pid=>$item){
       ...
       if(!empty($lprices)):
           ...
           $discount_r +=  ((($item['price']+$prices)*$item['quantity'])*$opt['discount'][$role])/100;
			 ...
			 $discount_r +=  (($item['price']+$prices)*$item['quantity']);
       endif;
...
function odudes_get_cart_total(){   
    return number_format((odudes_get_cart_subtotal()-odudes_get_cart_discount()),2,".","");
}

function odudes_grand_total(){
    $tax=odudes_calculate_tax();
    return number_format((odudes_get_cart_subtotal()+$tax['rate']-odudes_get_cart_discount()),2,".","");
}
//shipping calculation
function odudes_calculate_shipping(){
    $ship=array();
    $order = new Order();
...
//tax calculation
function odudes_calculate_tax(){
    $cartsubtotal=odudes_get_cart_subtotal();
    $taxr=array();
    $order = new Order();
    $order_info=$order->GetOrder($_SESSION['orderid']);
...
function odudes_checkout()
{
        wp_enqueue_script('jquery');
        $settings = get_option('_odudes_settings'); 
		
		if(isset($settings['msg_checkout_top']))
		  $msg_checkout_top=$settings['msg_checkout_top'] ;
	  else
		  $msg_checkout_top="";
		
        include(WP_PLUGIN_DIR."/odudeshop/layout/checkout/checkout.php");
}

function odudes_addtocart_js(){
    if(get_option('odudes_ajaxed_addtocart',0)==0) return;
?>
  ...
  jQuery(function(){
       jQuery('.wpdm-pp-add-to-cart-link').click(function(){
            if(this.href!=''){
                ...
                jQuery(this).html('<img src="<?php echo plugins_url();?>/odudeshop/images/wait.gif"/> adding...');
...
function odudes_buynow($content){    
    global $wpdb, $post, $wp_query, $current_user;    
    $settings = maybe_unserialize(get_option('_odudes_settings'));
    if(!isset($wp_query->query_vars['odudeshop'])||$wp_query->query_vars['odudeshop']==''||!isset($_REQUEST['buy'])||$_REQUEST['buy']=='')
    return $content;    
    @extract(get_post_meta($post->ID,"odudes_list_opts",true));
    odudes_add_to_cart($post->ID, $_REQUEST['buy']);    
    return '';
}
...
function update_os(){
    ...
    $wpdb->update("{$wpdb->prefix}os_orders",array('order_status'=>$_POST['status']),array('order_id'=>$_POST['order_id']));
    ...
    if($settings['stock']['enable']==1){  
        if($_POST['status']=="Completed"){
            if($settings['stock']['reduce_auto']==1)
                odudes_reduce_stock($_POST['order_id']);
        }
    } 
    
    $siteurl=home_url("/");
    //email to customer of that order
    $userid=$wpdb->get_var("select uid from {$wpdb->prefix}os_orders where order_id='".$_POST['order_id']."'");
    ...
    //print_r($email);   
    die(__('Order status updated',"odudeshop"));
}

function update_ps(){
    global $wpdb;
    $wpdb->update("{$wpdb->prefix}os_orders",array('payment_status'=>$_POST['status']),array('order_id'=>$_POST['order_id']));
    die(__('Payment status updated',"odudeshop"));
}

function ajaxinit(){
if(isset($_POST['action']) && $_POST['action']=='odudes_pp_ajax_call'){    
    if(function_exists($_POST['execute']) && $_POST['execute'] === 'PayNow')
        call_user_func('PayNow',$_POST);
        else
        echo __("function not defined!","odudeshop");
...
function ProcessOrder(){                                                                       
    ...
    if(preg_match("@\/payment\/([^\/]+)\/([^\/]+)@is",$_SERVER['REQUEST_URI'],$process)){
        ...
        $myorders = get_option('_odudes_users_orders',true);
        if($page=='notify'){
        if(!$uid) {
...
function odudes_clear_user_cartdata($user_login, $user) {
   delete_option($user->ID."_cart");
}
add_action('wp_login', 'odudes_clear_user_cartdata', 10, 2);


function odudes_array_diff($a, $b){
//    echo "<pre>";
//    print_r($a);
//    print_r($b);
//    echo "</pre>";
